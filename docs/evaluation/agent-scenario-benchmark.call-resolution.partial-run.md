...
生成时间：2026-09-24T14:07:12+08:00。FastAPI Agent API：`http://127.0.0.1:19002`；模型：`qwen2.5-coder:14b`。
...
基线场景 8；实际 Agent 调用 12；Planner 通过 8；Evidence Grounding 通过 5；有效 FinalDiagnosis 5；Ollama/格式错误 0。

任务类型：{"RUNTIME_ERROR": 2, "BUSINESS_LOGIC_ERROR": 2, "CALL_CHAIN_ANALYSIS": 2, "CONFIGURATION_ERROR": 2}。
状态：{"PARTIAL": 5, "FAIL": 1, "UNSUPPORTED": 2}。
...
- 不支持/校验问题：["证据未见于成功 Observation：src/OrderService.java:5", "模型行号 src/PriceService.java:5 与 Observation 不符，已按源码修正为 8", "模型定位行号与源码证据不符，已按已核验代码行修正", "调用链含有未观察到的已解析关系；仅保留已核验前缀", "模型声称确定的正确业务公式，但 Observation 中没有业务规则", "EVIDENCE_PROJECTED_FROM_TOOL: 1 verified facts", "EVIDENCE_NOT_FOUND: 模型原稿包含无法核验的源码", "CALL_EDGE_NOT_VERIFIED: 模型原稿包含无法核验的调用边", "MODEL_LINE_MISMATCH: 模型行号已由 Observation 校正", "UNSUPPORTED_CLAIM_REMOVED: 无需求证据的公式断言已移除"]
- Agent 状态：`COMPLETED`；诊断状态：`VALID`；用户提示：无。
...
- FinalDiagnosis：null
- 不支持/校验问题：["证据未见于成功 Observation：src/UserService.java:6", "证据未见于成功 Observation：src/EmailService.java:6", "证据未见于成功 Observation：src/TemplateService.java:6", "MODEL_LINE_MISMATCH: 使用 Observation 中的真实位置", "MODEL_SYMBOL_MISMATCH: 使用 Observation 中的 Symbol", "EVIDENCE_NOT_FOUND: 模型原稿包含无法核验的源码"]
- Agent 状态：`COMPLETED`；诊断状态：`INSUFFICIENT_EVIDENCE`；用户提示：当前工具返回的证据不足以形成可靠诊断；请展开分析过程查看已取得的证据。。
...
- 不支持/校验问题：["MODEL_LINE_MISMATCH: 使用 Observation 中的真实位置", "MODEL_SYMBOL_MISMATCH: 使用 Observation 中的 Symbol"]
- Agent 状态：`COMPLETED`；诊断状态：`VALID`；用户提示：无。
- 错误：无；类别：`{"http_502": false, "empty_response": false, "invalid_json": false, "tool_call_parse_error": false}`。备注：需核对全部已解析 CALLS 边。
...
| valid_final_diagnosis | 6 | 5 |
| evidence_grounding_pass | 6 | 5 |
...
- FinalDiagnosis：`{"issue_type": "BUSINESS_LOGIC_ERROR", "summary": "已定位价格计算实现；是否符合业务规则仍需确认", "location": {"file": "src/PriceService.java", "line": 8, "symbol": "PriceService.calculatePrice"}, "root_cause": "当前源码实现可能与预期业务规则不符；正确公式需要需求或测试用例确认。", "evidence": [{"file": "src/PriceService.java", "line": 8, "symbol": "PriceService.calculatePrice", "code": "return price + quantity * discount;", "reason": "readFile 返回的源码", "kind": "FACT", "source_tool": "readFile", "source_step": 7}, {"file": "src/OrderService.java", "line": 6, "symbol": "OrderService.createOrder", "code": "return priceService.calculatePrice(100, 3);", "reason": "readFile 返回的源码", "kind": "FACT", "source_tool": "readFile", "source_step": 2}], "call_chain": [], "fix_suggestion": "核对单价、数量、折扣的业务规则与测试用例，再调整计算表达式。", "uncertainty": "现有源码可确认当前计算表达式，但未提供预期计价规则。", "root_cause_kind": "INFERENCE"}`
- Unsupported Claims / 校验问题：`["证据未见于成功 Observation：src/OrderService.java:5", "模型行号 src/PriceService.java:5 与 Observation 不符，已按源码修正为 8", "模型定位行号与源码证据不符，已按已核验代码行修正", "调用链含有未观察到的已解析关系；仅保留已核验前缀", "模型声称确定的正确业务公式，但 Observation 中没有业务规则", "EVIDENCE_PROJECTED_FROM_TOOL: 1 verified facts", "EVIDENCE_NOT_FOUND: 模型原稿包含无法核验的源码", "CALL_EDGE_NOT_VERIFIED: 模型原稿包含无法核验的调用边", "MODEL_LINE_MISMATCH: 模型行号已由 Observation 校正", "UNSUPPORTED_CLAIM_REMOVED: 无需求证据的公式断言已移除"]`
- 备注：需同时看调用方与计算方法。

#### callchain-registration · PARTIAL → FAIL
...
- FinalDiagnosis：`null`
- Unsupported Claims / 校验问题：`["证据未见于成功 Observation：src/UserService.java:6", "证据未见于成功 Observation：src/EmailService.java:6", "证据未见于成功 Observation：src/TemplateService.java:6", "MODEL_LINE_MISMATCH: 使用 Observation 中的真实位置", "MODEL_SYMBOL_MISMATCH: 使用 Observation 中的 Symbol", "EVIDENCE_NOT_FOUND: 模型原稿包含无法核验的源码"]`
- 备注：需核对全部已解析 CALLS 边。
...
- FinalDiagnosis：`{"issue_type": "CALL_CHAIN_ANALYSIS", "summary": "追踪目标 Symbol 的真实调用路径", "location": {"file": "InventoryRepository.java", "line": 4, "symbol": "demo.order.InventoryRepository.findStock"}, "root_cause": "调用路径已通过 AST_RESOLVED_CALL_GRAPH 验证", "evidence": [{"file": "OrderController.java", "line": 11, "symbol": "demo.order.OrderController.createOrder", "code": "orderService.createOrder()", "reason": "已解析的 CALLS 关系", "kind": "FACT", "source_tool": "call_relation", "source_step": 5}, {"file": "OrderService.java", "line": 11, "symbol": "demo.order.OrderService.createOrder", "code": "inventoryService.checkStock()", "reason": "已解析的 CALLS 关系", "kind": "FACT", "source_tool": "call_relation", "source_step": 4}, {"file": "InventoryService.java", "line": 11, "symbol": "demo.order.InventoryService.checkStock", "code": "inventoryRepository.findStock()", "reason": "已解析的 CALLS 关系", "kind": "FACT", "source_tool": "call_relation", "source_step": 3}], "call_chain": ["OrderController.createOrder", "OrderService.createOrder", "InventoryService.checkStock", "InventoryRepository.findStock"], "fix_suggestion": "", "uncertainty": "", "root_cause_kind": "INFERENCE"}`
- Unsupported Claims / 校验问题：`["MODEL_LINE_MISMATCH: 使用 Observation 中的真实位置", "MODEL_SYMBOL_MISMATCH: 使用 Observation 中的 Symbol"]`
- 备注：需核对全部已解析 CALLS 边。
...
- Diagnosis Issues：`["证据未见于成功 Observation：src/OrderService.java:5", "模型行号 src/PriceService.java:5 与 Observation 不符，已按源码修正为 8", "模型定位行号与源码证据不符，已按已核验代码行修正", "调用链含有未观察到的已解析关系；仅保留已核验前缀", "模型声称确定的正确业务公式，但 Observation 中没有业务规则", "EVIDENCE_PROJECTED_FROM_TOOL: 1 verified facts", "EVIDENCE_NOT_FOUND: 模型原稿包含无法核验的源码", "CALL_EDGE_NOT_VERIFIED: 模型原稿包含无法核验的调用边", "MODEL_LINE_MISMATCH: 模型行号已由 Observation 校正", "UNSUPPORTED_CLAIM_REMOVED: 无需求证据的公式断言已移除"]`
- FinalDiagnosis：`{"issue_type": "BUSINESS_LOGIC_ERROR", "summary": "已定位价格计算实现；是否符合业务规则仍需确认", "location": {"file": "src/PriceService.java", "line": 8, "symbol": "PriceService.calculatePrice"}, "root_cause": "当前源码实现可能与预期业务规则不符；正确公式需要需求或测试用例确认。", "evidence": [{"file": "src/PriceService.java", "line": 8, "symbol": "PriceService.calculatePrice", "code": "return price + quantity * discount;", "reason": "readFile 返回的源码", "kind": "FACT", "source_tool": "readFile", "source_step": 7}, {"file": "src/OrderService.java", "line": 6, "symbol": "OrderService.createOrder", "code": "return priceService.calculatePrice(100, 3);", "reason": "readFile 返回的源码", "kind": "FACT", "source_tool": "readFile", "source_step": 2}], "call_chain": [], "fix_suggestion": "核对单价、数量、折扣的业务规则与测试用例，再调整计算表达式。", "uncertainty": "现有源码可确认当前计算表达式，但未提供预期计价规则。", "root_cause_kind": "INFERENCE"}`
...
- System Corrected Result：`{"location": null, "call_chain": [], "projected_fact_count": 0}`
- Diagnosis Issues：`["证据未见于成功 Observation：src/UserService.java:6", "证据未见于成功 Observation：src/EmailService.java:6", "证据未见于成功 Observation：src/TemplateService.java:6", "MODEL_LINE_MISMATCH: 使用 Observation 中的真实位置", "MODEL_SYMBOL_MISMATCH: 使用 Observation 中的 Symbol", "EVIDENCE_NOT_FOUND: 模型原稿包含无法核验的源码"]`
- FinalDiagnosis：`null`
...
- System Corrected Result：`{"location": {"file": "InventoryRepository.java", "line": 4, "symbol": "demo.order.InventoryRepository.findStock"}, "call_chain": ["OrderController.createOrder", "OrderService.createOrder", "InventoryService.checkStock", "InventoryRepository.findStock"], "projected_fact_count": 0}`
- Diagnosis Issues：`["MODEL_LINE_MISMATCH: 使用 Observation 中的真实位置", "MODEL_SYMBOL_MISMATCH: 使用 Observation 中的 Symbol"]`
- FinalDiagnosis：`{"issue_type": "CALL_CHAIN_ANALYSIS", "summary": "追踪目标 Symbol 的真实调用路径", "location": {"file": "InventoryRepository.java", "line": 4, "symbol": "demo.order.InventoryRepository.findStock"}, "root_cause": "调用路径已通过 AST_RESOLVED_CALL_GRAPH 验证", "evidence": [{"file": "OrderController.java", "line": 11, "symbol": "demo.order.OrderController.createOrder", "code": "orderService.createOrder()", "reason": "已解析的 CALLS 关系", "kind": "FACT", "source_tool": "call_relation", "source_step": 5}, {"file": "OrderService.java", "line": 11, "symbol": "demo.order.OrderService.createOrder", "code": "inventoryService.checkStock()", "reason": "已解析的 CALLS 关系", "kind": "FACT", "source_tool": "call_relation", "source_step": 4}, {"file": "InventoryService.java", "line": 11, "symbol": "demo.order.InventoryService.checkStock", "code": "inventoryRepository.findStock()", "reason": "已解析的 CALLS 关系", "kind": "FACT", "source_tool": "call_relation", "source_step": 3}], "call_chain": ["OrderController.createOrder", "OrderService.createOrder", "InventoryService.checkStock", "InventoryRepository.findStock"], "fix_suggestion": "", "uncertainty": "", "root_cause_kind": "INFERENCE"}`
