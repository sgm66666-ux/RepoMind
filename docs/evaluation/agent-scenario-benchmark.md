...
生成时间：2026-09-24T00:34:55+08:00。FastAPI Agent API：`http://127.0.0.1:19002`；模型：`qwen2.5-coder:14b`。
...
基线场景 8；实际 Agent 调用 20；Planner 通过 8；Evidence Grounding 通过 3；有效 FinalDiagnosis 3；Ollama/格式错误 0。

任务类型：{"RUNTIME_ERROR": 2, "BUSINESS_LOGIC_ERROR": 2, "CALL_CHAIN_ANALYSIS": 2, "CONFIGURATION_ERROR": 2}。
状态：{"PARTIAL": 3, "FAIL": 3, "UNSUPPORTED": 2}。
...
- 目标 Symbol：`UserService.login`；未由 searchSymbol 确认。
...
- FinalDiagnosis：null
- 不支持/校验问题：[]
- Agent 状态：`MAX_STEPS`；诊断状态：`NOT_COMPLETED`；用户提示：已达到工具调用步数上限，尚无最终诊断。。
...
### callchain-order-stock · baseline #1 · FAIL
...
- FinalDiagnosis：null
- 不支持/校验问题：["定位文件未见于 Observation", "定位 Symbol 未见于 Observation", "定位行号未见于源码或已解析调用 Observation", "调用链含有未观察到的已解析关系", "未取得可核验的完整调用边，不能声称调用路径已追踪"]
...
- 证据：[]
- FinalDiagnosis：null
- 不支持/校验问题：["Agent 达到 maxSteps，未形成配置文件与使用位置的证据链"]
- Agent 状态：`MAX_STEPS`；诊断状态：`UNSUPPORTED`；用户提示：当前工具链无法读取并关联配置文件与代码使用位置，暂不能确认配置故障原因。 Agent 已达到工具调用步数上限。。
